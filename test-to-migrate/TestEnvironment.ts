...
import { configure, RelayProvider } from '@rsksmart/rif-relay-client';
...
class TestEnvironmentClass {
    ...
    async start(
        ...
        if (_host == null) {
            ...
            throw new Error(
                `start: expected network (${supportedNetworks().join(
                    '|'
                )}) or url`
            );
        }
        const commandsLogic = new TestSetup(
            _host,
            configure({ chainId: environment.chainId })
        );
        ...
        const registrationResult = await commandsLogic.registerRelay(
            registerOptions
        );
        if (registrationResult.success) {
            console.log(
                'In-process relay successfully registered:',
                JSON.stringify(registrationResult)
            );
        } else {
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            throw new Error(
                `Failed to fund relay: ${
                    registrationResult.error
                } : ${registrationResult?.transactions?.toString()}`
            );
        }
        ...
        const relayProvider = new RelayProvider(
            new Web3.providers.HttpProvider(_host),
            config
        );
    ...
    async _runServer(
        ...
        this.httpServer = new HttpServer(port, relayServer);
