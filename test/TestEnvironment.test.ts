...
contract('TestEnvironment', function (accounts) {
    describe('#start()', function () {
        it('should create a valid test environment for other tests to rely on', async function () {
            ...
            const testEnv = await TestEnvironment.start(
                host,
                0.6e18,
                await getTestingEnvironment()
            );
    ...
    describe('using RelayClient', () => {
        ...
        before(async () => {
            const host =
                (web3.currentProvider as HttpProvider).host ?? 'localhost';
            tToken = await TestToken.new();
            testEnvironment = await TestEnvironment.start(
                host,
                0.6e18,
                await getTestingEnvironment()
            );
            const dVerifier = await DeployVerifier.at(
                testEnvironment.deploymentResult.deployVerifierAddress
            );
            await dVerifier.acceptToken(tToken.address, { from: accounts[0] });
            const rVerifier = await RelayVerifier.at(
                testEnvironment.deploymentResult.relayVerifierAddress
            );
        ...
        it('should relay using relayTransaction', async () => {
            ...
            const smartWalletFactory: SmartWalletFactoryInstance =
                await SmartWalletFactory.at(
                    testEnvironment.deploymentResult.smartWalletFactoryAddress
                );
            const sr: TestRecipientInstance = await TestRecipient.new();

            testEnvironment.relayProvider.relayClient.accountManager.addAccount(
                sender
            );
            ...
            const wallet = await smartWalletFactory.getSmartWalletAddress(
                sender.address,
                constants.ZERO_ADDRESS,
                '0'
            );
            const ret =
                await testEnvironment.relayProvider.relayClient.relayTransaction(
                    {
                        from: sender.address,
                        to: sr.address,
                        callForwarder: wallet,
                        callVerifier:
                            testEnvironment.deploymentResult
                                .relayVerifierAddress,
                        // gas: '0x' + 1e6.toString(16),
                        data: sr.contract.methods
                            .emitMessage('hello')
                            .encodeABI(),
                        tokenAmount: '0x00',
                        tokenGas: '0',
                        ...
                        clientId: '1',
                        relayHub:
                            testEnvironment.deploymentResult.relayHubAddress
                    }
                );

            assert.deepEqual(
                [...ret.relayingErrors.values(), ...ret.pingErrors.values()],
                []
            );
            const events = await sr.contract.getPastEvents();
            assert.equal(events[0].event, 'SampleRecipientEmitted');
            assert.equal(
                events[0].returnValues.msgSender.toLowerCase(),
                wallet.toLowerCase()
            );
        });
    });
