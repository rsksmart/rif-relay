    ...
    contract(params.title + 'Flow', function (accounts) {
        ...
        if (params.relay) {
            before(params.title + 'enable relay', async function () {
                ...
                const sWalletTemplate: SmartWalletInstance =
                    await SmartWallet.new();
                const factory: SmartWalletFactoryInstance =
                    await createSmartWalletFactory(sWalletTemplate);
                const smartWalletInstance: SmartWalletInstance =
        ...
        it(params.title + 'send normal transaction', async () => {
            ...
            let res;
            try {
                const gas = await sr.contract.methods
                    .emitMessage('hello')
                    .estimateGas();
                res = params.relay
                    ? await sr.emitMessage('hello', { from })
                    : await sr.emitMessage('hello', { from, gas });
            } catch (e) {
        ...
        it(params.title + 'send gasless transaction', async () => {
            ...
            console.log(
                'running gasless-emitMessage (should fail for direct, succeed for relayed)'
            );
            let ex: Error | undefined;
            try {
                const res = await sr.emitMessage('hello, from gasless', {
                    from: gaslessAccount.address,
                    gas: 1e6
                });
                console.log(
                    'res after gasless emit:',
            ...
            if (params.relay) {
                ...
                assert.ok(
                    ex == null,
                    `should succeed sending gasless transaction through relay. got: ${ex?.toString()}`
                );
            } else {
                ...
                assert.ok(
                    ...
                    `Expected Error with 'insufficient funds'. got: ${ex?.toString()}`
                );
            }
        });

        it(
            params.title + 'running testRevert (should always fail)',
            async () => {
        ...
        async function asyncShouldThrow(
            ...
            assert.ok(
                isExpectedError,
                `Expected to throw ${msg} but threw ${ex?.message}`
            );
        }
    });
});
