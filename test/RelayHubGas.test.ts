  ...
  describe('relayCall', function () {
    ...
    context('with staked and registered relay', function () {
      ...
      context('with funded verifier', function () {
        ...
        function signRequest (request: RelayRequest, sWalletAddress: string): string {
          const reqToSign = new TypedRequestData(
            chainId,
            sWalletAddress,
            request
        ...
        async function createSmartWalletInstance (): Promise<SmartWalletInstance> {
          const SmartWallet = artifacts.require('SmartWallet')
          const smartWalletTemplate: SmartWalletInstance =
                        await SmartWallet.new()
          const smartWalletFactory: SmartWalletFactoryInstance =
        ...
        function logGasOverhead (gasOverhead: number): void {
        ...
        async function printGasStatus (txReceipt: TransactionReceipt): Promise<void> {
        ...
        async function estimateGasOverhead (fees: string): Promise<void> {
          ...
          const logs = abiDecoder.decodeLogs(txReceipt.logs)
          const findLog = (logs: any, name: string): void =>
