...
import RelayRequest, { cloneRelayRequest } from '../src/common/EIP712/EnvelopingRequest'
...
import {
  ...
  TestPaymasterConfigurableMisbehaviorInstance, EnvelopingHubInstance, TestTokenContract, TestTokenInstance, ProxyFactoryInstance, SmartWalletInstance
} from '../types/truffle-contracts'
import { createProxyFactory, createSmartWallet, deployEnvelopingHub, deployHub, encodeRevertReason, getTestingEnvironment } from './TestUtils'
import { registerForwarderForGsn, registerSmartWalletForEnveloping } from '../src/common/EIP712/ForwarderUtil'
...
import EnvelopingTypedRequestData from '../src/common/EIP712/EnvelopingTypedRequestData'
...
contract('EnvelopingHub', function ([_, relayOwner, relayManager, relayWorker, other, dest, incorrectWorker]) { // eslint-disable-line no-unused-vars
  ...
  let chainId: number
  let envelopingHub: string
  let stakeManager: StakeManagerInstance
  let penalizer: PenalizerInstance
  let envelopingHubInstance: EnvelopingHubInstance
  ...
  beforeEach(async function () {
    ...
    envelopingHubInstance = await deployEnvelopingHub(stakeManager.address, penalizer.address)
    ...
    smartWallet = sw.address
    recipientContract = await TestRecipient.new(smartWallet)


    // register hub's RelayRequest with forwarder, if not already done.
    await registerSmartWalletForEnveloping(sw)

    target = recipientContract.address
    paymaster = paymasterContract.address
    envelopingHub = envelopingHubInstance.address
    await tokenContract.mint('1000', smartWallet)

    // await paymasterContract.setTrustedForwarder(smartWallet)
    // await paymasterContract.setRelayHub(relayHub)
  })

  it('should retrieve version number', async function () {
    const version = await envelopingHubInstance.versionHub()
  ...
  describe('balances', function () {
    async function testDeposit (sender: string, paymaster: string, amount: BN): Promise<void> {
      ...
      const relayHubBalanceTracker = await balance.tracker(envelopingHub)

      // TODO review gas price for RSK
      const { logs } = await envelopingHubInstance.depositFor(paymaster, {
        from: sender,
        value: amount,
        gasPrice: 1
      })
      ...
      expect(await envelopingHubInstance.balanceOf(paymaster)).to.be.bignumber.equal(amount)
    ...
    it('cannot deposit amounts larger than the limit', async function () {
      await expectRevert.unspecified(
        envelopingHubInstance.depositFor(target, {
          from: other,
          value: ether('3'),
          gasPrice: 1
        }),
    ...
    it('can deposit multiple times and have a total deposit larger than the limit', async function () {
      await envelopingHubInstance.depositFor(target, {
        from: other,
        value: ether('1'),
        gasPrice: 1
      })
      ...
      expect(await envelopingHubInstance.balanceOf(target)).to.be.bignumber.equals(ether('3'))
    })
    ...
    it('accounts with deposits can withdraw partially', async function () {
      ...
      const { logs } = await envelopingHubInstance.withdraw(amount.divn(2), dest, { from: other })
    ...
    it('accounts with deposits can withdraw all their balance', async function () {
      ...
      const { logs } = await envelopingHubInstance.withdraw(amount, dest, { from: other })
      expectEvent.inLogs(logs, 'Withdrawn', {
        account: other,
        dest,
        amount
    ...
    it('accounts cannot withdraw more than their balance', async function () {
      ...
      await expectRevert.unspecified(envelopingHubInstance.withdraw(amount.addn(1), dest, { from: other }), 'insufficient funds')
    })
  })
  ...
  describe('relayCall', function () {
    ...
    const senderNonce = '0'
    let sharedRelayRequestData: EnvelopingRequest
    ...
    context('with unknown worker', function () {
      ...
      it('should not accept a relay call', async function () {
        await expectRevert.unspecified(
          envelopingHubInstance.relayCall(10e6, relayRequest, signature, approvalData, gas, {
            from: relayWorker,
            gas
          }),
      ...
      context('with manager stake unlocked', function () {
        beforeEach(async function () {
          ...
          await stakeManager.authorizeHubByOwner(relayManager, envelopingHub, { from: relayOwner })
          await envelopingHubInstance.addRelayWorkers([relayWorker], {
            from: relayManager
          })
          await stakeManager.unauthorizeHubByOwner(relayManager, envelopingHub, { from: relayOwner })
        })
        it('should not accept a relay call', async function () {
          await expectRevert.unspecified(
            envelopingHubInstance.relayCall(10e6, relayRequest, signature, approvalData, gas, {
    ...
    context('with staked and registered relay', function () {
      ...
      beforeEach(async function () {
        ...
        await envelopingHubInstance.addRelayWorkers([relayWorker], { from: relayManager })
        await envelopingHubInstance.registerRelayServer(baseRelayFee, pctRelayFee, url, { from: relayManager })
        relayRequest = cloneRelayRequest(sharedRelayRequestData)
        relayRequest.request.data = encodedFunction
        const dataToSign = new EnvelopingTypedRequestData(
          chainId,
          smartWallet,
          relayRequest
        )
      ...
      context('with relay worker that is not externally-owned account', function () {
        it('should not accept relay requests', async function () {
          ...
          await envelopingHubInstance.addRelayWorkers([testRelayWorkerContract.address], {
            from: relayManager
          })
          await expectRevert.unspecified(
            testRelayWorkerContract.relayCall(
              envelopingHubInstance.address,
              10e6,
              relayRequest,
              signature,
              gas,
...
//           await envelopingHubInstance.depositFor(misbehavingPaymaster.address, {
//             value: ether('1'),
//             from: other
//           })
...
//           const relayCallView = await envelopingHubInstance.contract.methods.relayCall(
//             10e6,
//             relayRequest,
...
//           const relayCallView =
//             await envelopingHubInstance.contract.methods
...
//           await misbehavingPaymaster.setRelayHub(relayHub)
//           await envelopingHubInstance.depositFor(paymasterWithContext.address, {
//             value: ether('1'),
//             from: other
//           })
//           await envelopingHubInstance.depositFor(misbehavingPaymaster.address, {
...
//           const { tx, logs } = await envelopingHubInstance.relayCall(10e6, relayRequest, signatureWithPermissivePaymaster, '0x', gas, {
//             from: relayWorker,
//             gas,
//             gasPrice
//           })
...
//           const { tx } = await envelopingHubInstance.relayCall(10e6, relayRequest, signatureWithPermissivePaymaster, '0x', gas, {
//             from: relayWorker,
//             gas,
//             gasPrice
//           })
...
//           const ret = await envelopingHubInstance.relayCall(10e6, relayRequest, signatureWithPermissivePaymaster, '0x', gas, {
//             from: relayWorker,
//             gas,
//             gasPrice
//           })
...
//             dataToSign
//           )
//           const { tx } = await envelopingHubInstance.relayCall(10e6, relayRequestNoCallData, signature, '0x', gas, {
//             from: relayWorker,
//             gas,
//             gasPrice
//           })
...
//             dataToSign
//           )
//           const { logs } = await envelopingHubInstance.relayCall(10e6, relayRequestRevert, signature, '0x', gas, {
//             from: relayWorker,
//             gas,
//             gasPrice
//           })
...
//           const { tx } = await envelopingHubInstance.relayCall(10e6, relayRequestPaymasterWithContext,
...
//           await misbehavingPaymaster.setReturnInvalidErrorCode(true)
//           const { logs } = await envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster,
...
//           await expectRevert.unspecified(
//             envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', gas, {
//               from: relayWorker,
//               gasPrice,
//               gas
//             }),
...
//           await expectRevert.unspecified(
//             envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', 100000001, {
//               from: relayWorker,
//               gasPrice,
//               gas
//             }),
...
//           await envelopingHubInstance.addRelayWorkers([incorrectWorker], { from: relayManager })
//           await expectRevert.unspecified(
//             envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', gas, {
...
//             const maxPossibleCharge = (await envelopingHubInstance.calculateCharge(gasLimit, {
//               gasPrice,
//               pctRelayFee,
//               baseRelayFee,
//               relayWorker,
...
//             await expectRevert.unspecified(
//               envelopingHubInstance.relayCall(10e6, relayRequestPaymaster2, signatureWithMisbehavingPaymaster, '0x', gas, {
//                 from: relayWorker,
//                 gas,
//                 gasPrice
//               }),
...
//           await expectRevert.unspecified(envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster,
//             signatureWithMisbehavingPaymaster, '0x', gas, {
//               from: relayWorker,
//               gas,
//               gasPrice: gasPrice
...
//             const { logs } = await envelopingHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signature, '0x', gas, {
//               from: relayWorker,
//               gas,
//               gasPrice
//             })
