...
contract('Network Simulation for Relay Server', function (accounts) {
    ...
    before(async function () {
        provider = new NetworkSimulatingProvider(
    ...
    describe('without automated mining', function () {
        ...
        it('should broadcast multiple transactions at once', async function () {
            ...
            const overrideDetails: Partial<EnvelopingTransactionDetails> = {
                from: gaslessAccount.address,
                callForwarder: smartWallet.address
            };
            // noinspection ES6MissingAwait - done on purpose
            const promises = [
                env.relayTransaction(false),
                env.relayTransaction(false, overrideDetails)
            ];
            ...
            await env.assertTransactionRelayed(
                txs[0].txHash,
                txs[0].reqSigHash
            );
            await env.assertTransactionRelayed(
