...
context('#ServerConfigParams', () => {
    ...
    context('#parseServerConfig', () => {
        ...
        it('cmdline should override env, which should override file', async () => {
            ...
            assert.deepInclude(
                parseServerConfig(
                    ['--config', tmpConfigfile, '--url', 'cmdparam'],
                    env
                ),
                { url: 'cmdparam' }
            );
        ...
        it('should throw on unknown cmdline param', function () {
            expectThrow(
                () => parseServerConfig(['--asdasd'], {}),
                'unexpected param asdasd'
            );
        });

        it('should throw on missing config file', function () {
            expectThrow(
                () => parseServerConfig(['--config=nosuchfile'], {}),
    ...
    context('#resolveServerConfig', () => {
        ...
        it('should fail on invalid relayhub address', async () => {
            ...
            await expectRevert(
                resolveServerConfig(config, provider),
                'invalid param: "relayHubAddress" is not a valid address: 123'
            );
        });
        ...
        it('should fail on missing hubid for VersionRegistry', async () => {
            ...
            await expectRevert(
                resolveServerConfig(config, provider),
                'missing param: relayHubId to read from VersionRegistry'
            );
        });

        it('should fail on no-contract VersionRegistry address', async () => {
            const config = {
                versionRegistryAddress: addr(1),
                relayHubId: 'hubid'
            };
            await expectRevert(
                resolveServerConfig(config, provider),
        ...
        contract('with VersionRegistry', () => {
            ...
            before(async () => {
                ...
                await oracle.addVersion(
                    string32('hub-invalidaddr'),
                    string32('1.0'),
                    'garbagevalue'
                );
                await oracle.addVersion(
                    string32('hub-nocontract'),
                    string32('1.0'),
                    addr(2)
                );
                await oracle.addVersion(
                    string32('hub-wrongcontract'),
                    string32('1.0'),
                    oracle.address
                );
            });
            ...
            it('should fail on invalid hub address in oracle', async () => {
                const config = {
                    versionRegistryAddress: oracle.address,
                    relayHubId: 'hub-invalidaddr'
                };
                await expectRevert(
                    resolveServerConfig(config, provider),
