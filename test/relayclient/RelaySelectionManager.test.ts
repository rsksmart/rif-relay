...
describe('RelaySelectionManager', async function () {
  const sliceSize = 3
  const dependencyTree = getDependencies(configure({}), ethers.provider)
  ...
  describe('#selectNextRelay()', function () {
    ...
    describe('with preferred relay URL', function () {
      ...
      before(async function () {
        ...
        await stake(relayHub, relayManager, await ethers.getSigner(accounts[0]))
        await register(relayHub, await ethers.getSigner(relayManager), accounts[2], preferredRelayUrl)

        const config = configure({
          relayHubAddress: relayHub.address,
          chainId
        })
        dependencyTree = getDependencies(config, ethers.provider)
