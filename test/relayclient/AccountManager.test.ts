...
contract('AccountManager', function () {
    ...
    describe('#addAccount()', function () {
        it('should save the provided keypair internally', async function () {
            ...
            const accountManager = new AccountManager(
                ...
                async (signedData: any): Promise<string> => {
                    ...
                    // @ts-ignore
                    return sigUtil.signTypedData_v4(account.privateKey, {
                        data: signedData
                    });
                }
            );
        ...
        it('should throw if the provided keypair is not valid', async function () {
            ...
            const keypair = {
                privateKey: Buffer.from(
                    'AAAAAAAAAAAAA6a65a842ce7c78d9aa706d3b113bce9c46f30d7d21715b23b1d',
                    'hex'
                ),
                address
            };
    ...
    describe('#sign()', function () {
        ...
        function relayRequestWithoutExtraData(
        ...
        it('should use internally controlled keypair for signing if available', async function () {
            ...
            assert.ok(
                isSameAddress(relayRequest.request.from.toLowerCase(), rec)
            );
            expect(
                accountManager._signWithControlledKey
            ).to.have.been.calledWith(keypair, signedData);
        ...
        it('should ask provider to sign if key is not controlled', async function () {
            ...
            expect(accountManager._signWithProvider).to.have.been.calledWith(
                signedData
            );
            expect(
                accountManager._signWithControlledKey
        ...
        it('should throw if web3 fails to sign with requested address', async function () {
            ...
            relayRequest.request.from =
                '0x4cfb3f70bf6a80397c2e634e5bdd85bc0bb189ee';
            const promise = accountManager.sign(relayRequest);
            await expect(promise).to.be.eventually.rejectedWith(
                'Failed to sign relayed transaction for 0x4cfb3f70bf6a80397c2e634e5bdd85bc0bb189ee'
            );
        });
    });
});
