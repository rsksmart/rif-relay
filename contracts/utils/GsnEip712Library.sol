...
library GsnEip712Library {
  ...
  function deploy(GsnTypes.DeployRequest calldata relayRequest, bytes calldata signature) internal returns (bool deploySuccess) {
            ...
            (deploySuccess,) = relayRequest.relayData.callForwarder.call{gas: relayRequest.request.gas}(
                ...
                keccak256("RelayRequest(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes data,address tokenContract,uint256 tokenAmount,address recoverer,uint256 index,RelayData relayData)RelayData(bytes32 domainSeparator,address relayWorker,address callForwarder,address callVerifier)"),
    ...
    function execute(GsnTypes.RelayRequest calldata relayRequest, bytes calldata signature) internal returns (bool forwarderSuccess, uint256 lastSuccTx, bytes memory ret) {
            ...
            (forwarderSuccess, ret) = relayRequest.relayData.callForwarder.call(
                ...
                relayRequest.request, relayRequest.relayData.domainSeparator,
                keccak256("RelayRequest(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes data,address tokenContract,uint256 tokenAmount,RelayData relayData)RelayData(uint256 gasPrice,bytes32 domainSeparator,address relayWorker,address callForwarder,address callVerifier)"), 
                hashRelayData(relayRequest.relayData), signature
                ));
