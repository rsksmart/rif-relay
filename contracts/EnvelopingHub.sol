...
import "./interfaces/IEnvelopingHub.sol";
import "./interfaces/IPaymaster.sol";
import "./forwarder/IForwarder.sol";
import "./interfaces/IStakeManager.sol";

contract EnvelopingHub is IEnvelopingHub {
    ...
    function verifyGasLimits(
        ...
        GsnTypes.EnvelopingRequest calldata relayRequest,
        uint256 initialGas
    )
    private
    view
...

    {
        ...
        (bool success, bytes memory relayCallStatus) = address(this).call{gas:innerGasLimit}(
            abi.encodeWithSelector(EnvelopingHub.innerRelayCall.selector, relayRequest, signature, approvalData, vars.gasLimits,
    ...
    returns (RelayCallStatus, bytes memory)
    {
        ...
        // This external function can only be called by EnvelopingHub itself, creating an internal transaction. Calls to the
        // recipient (preRelayedCall, the relayedCall, and postRelayedCall) are called from inside this transaction.
        require(msg.sender == address(this), "Only EnvelopingHub should call this function");
        ...
        // The recipient is no allowed to withdraw balance from EnvelopingHub during a relayed transaction. We check pre and
...

        {
            ...
            uint256 lastSuccTrx;
            (forwarderSuccess, vars.relayedCallSuccess,lastSuccTrx, vars.relayedCallReturnValue) = GsnEip712Library.executeEnveloping(relayRequest, signature);
            if ( !forwarderSuccess ) {
