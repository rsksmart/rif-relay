...
const cloneRelayRequest = (relayRequest: RelayRequest, override: {
...
function logGasOverhead(gasOverhead: BigNumber) {
  console.log(`${bgMagenta}Enveloping Overhead Gas: ${gasOverhead.toString()}${reset}\n`);
}
...
const estimateGas = async () => {
  ...
  const deployAndSetup = async () => {
    ...
    return {
      forwarder, recipient, relayHub, owner, relayRequest
    }
  }
  ...
  let relayCallProcessFirstResult = await triggerRelayCallProcess({
    message,
  ...
  console.log('ROUND 2');
  let relayCallProcessSecondResult = await triggerRelayCallProcess({
    message,
  });
  ...
  message = 'RIF ENVELOPING '.repeat(77);
  relayCallProcessFirstResult = await triggerRelayCallProcess({
    message,
    estimateGasLimit: true,
    noPayment: true,
    noTokenGas: true,
  ...
  console.log('ROUND 2');
  relayCallProcessSecondResult = await triggerRelayCallProcess({
    message,
    noPayment: true,
    noTokenGas: true,
  });
  ...
  message = 'RIF Enveloping '.repeat(32);
  ({ recipient, forwarder, owner, relayRequest, relayHub } = await deployAndSetup());
