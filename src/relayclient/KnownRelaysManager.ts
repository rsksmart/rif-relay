...
export class KnownRelaysManager {
  ...
  async getRelayInfoForManagers (relayManagers: Set<Address>): Promise<RelayRegisteredEventInfo[]> {
    ...
    this.contractInteractor.decodeEvents(mergedEvents).forEach(event => {
      const args = event.args
      if (event.name === RelayServerRegistered) {
        const registeredEvent: RelayRegisteredEventInfo = { relayManager: args.relayManager, relayUrl: args.relayUrl }
        activeRelays.set(args.relayManager, registeredEvent)
      } else {
  ...
  async getPastEventsForHub (fromBlock: number, toBlock: number): Promise<LogDescription[]> {
    let relayEventParts: Event[]
    while (true) {
    ...
    return this.contractInteractor.decodeEvents(relayEventParts.flat())
  }
  ...
  async _fetchRecentlyActiveRelayManagers (): Promise<Set<Address>> {
    ...
    const relayEvents: LogDescription[] = await this.getPastEventsForHub(fromBlock, toBlock)
    log.info(`fetchRelaysAdded: found ${relayEvents.length} events`)
    const foundRelayManagers: Set<Address> = new Set()

    relayEvents.forEach((event: LogDescription) => {
