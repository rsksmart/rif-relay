...
import { Address, PingFilter, PrefixedHexString } from './types/Aliases'
...
import { calculateDeployTransactionMaxPossibleGas, decodeRevertReason, estimateMaxPossibleRelayCallWithLinearFit } from '../common/Utils'
...
export class RelayClient {
   ...
   */
  async _broadcastRawTx (transaction: Transaction, signedTx?: PrefixedHexString): Promise<{ hasReceipt: boolean, broadcastError?: Error, wrongNonce?: boolean }> {
    const rawTx = ethers.utils.serializeTransaction(transaction, signedTx)
    const txHash = ethers.utils.sha256(rawTx)
  ...
  async estimateMaxPossibleRelayGasWithLinearFit (transactionDetails: EnvelopingTransactionDetails, relayWorker: Address): Promise<number> {
    ...
    if (trxDetails.tokenGas === undefined || trxDetails.tokenGas == null) {
      ...
      trxDetails.tokenGas = ethers.utils.hexlify(tokenGas)
    } else {
      tokenGas = BigNumber.from(trxDetails.tokenGas).toNumber()
    }
    ...
    if (isSmartWalletDeploy) {
      ...
      if (trxDetails.gas === undefined || trxDetails.gas == null) {
        ...
        destinationGas = BigNumber.from(trxDetails.gas).toNumber()
      }
  ...
  async estimateMaxPossibleRelayGas (transactionDetails: EnvelopingTransactionDetails, relayWorker: Address): Promise<number> {
    ...
    if (isSmartWalletDeploy) {
      ...
      maxPossibleGas = BigNumber.from(Math.ceil(estimated * constants.ESTIMATED_GAS_CORRECTION_FACTOR))
    }
  ...
  async estimateTokenTransferGas (transactionDetails: EnvelopingTransactionDetails, relayWorker: Address): Promise<number> {
    ...
    if (tokenContract !== constants.ZERO_ADDRESS && BigNumber.from(transactionDetails.tokenAmount ?? '0').gt(BigNumber.from(0))) {
      ...
      if (tokenOrigin !== constants.ZERO_ADDRESS) {
        ...
        const encodedFunction = iface.encodeFunctionData('transfer', [relayWorker, transactionDetails.tokenAmount ?? '0'])
  ...
  async relayTransaction (transactionDetails: EnvelopingTransactionDetails): Promise<RelayingResult> {
    ...
    if (transactionDetails.isSmartWalletDeploy ?? false) {
      ...
      transactionDetails.gas = ethers.utils.hexlify(internalCallCost)
    }
  ...
  async _attemptRelay (
    ...
    let signedTransaction: PrefixedHexString
    this.emit(new SendToRelayerEvent(relayInfo.relayInfo.relayUrl))
    try {
      signedTransaction = await this.httpClient.relayTransaction(relayInfo.relayInfo.relayUrl, httpRequest)
    } catch (error) {
    ...
    const transactionParameters = parseTransaction(signedTransaction)
    const transaction: Transaction = { ...transactionParameters, chainId: this.contractInteractor.getRawTxOptions() }

    if (!(await this.transactionValidator.validateRelayResponse(httpRequest, signedTransaction))) {
    ...
    await this._broadcastRawTx(transaction, signedTransaction)
    return {
      transaction
    }
  }
  ...
  getEstimateGasParams (transactionDetails: EnvelopingTransactionDetails): EstimateGasParams {
    const params: EstimateGasParams = {
