 ...
 * @param partialConfig
 */
export async function resolveConfiguration (provider: providers.JsonRpcProvider, partialConfig: Partial<EnvelopingConfig>): Promise<EnvelopingConfig> {
  // @ts-ignore
...
export function getDependencies (config: EnvelopingConfig, provider?: providers.JsonRpcProvider, overrideDependencies?: Partial<EnvelopingDependencies>): EnvelopingDependencies {
  ...
  if (contractInteractor == null) {
    if (provider != null) {
      ...
      throw new Error('either contract interactor or provider must be non-null')
    }
  }
  ...
  if (accountManager == null) {
    if (provider != null) {
      ...
      throw new Error('either account manager or provider must be non-null')
    }
  }
