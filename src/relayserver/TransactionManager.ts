...
`)
  ...
  async boostUnderpricedPendingTransactionsForSigner (signer: string, currentBlockHeight: number): Promise<Map<PrefixedHexString, SignedTransactionDetails>> {
    ...
    if (oldestPendingTx.nonce < nonce) {
      log.debug(`${signer} : transaction is mined, awaiting confirmations. Account nonce: ${nonce}, oldest transaction: nonce: ${oldestPendingTx.nonce} txId: ${oldestPendingTx.txId}`)
      return boostedTransactions
    }
    ...
    if (currentBlockHeight - lastSentAtBlockHeight < this.config.pendingTransactionTimeoutBlocks) {
      log.debug(`${signer} : awaiting transaction with ID: ${oldestPendingTx.txId} to be mined. creationBlockNumber: ${oldestPendingTx.creationBlockNumber} nonce: ${nonce}`)
      return boostedTransactions
    }
    ...
    for (const transaction of underpricedTransactions) {
      ...
      log.debug(`Replaced transaction: nonce: ${transaction.nonce} sender: ${signer} | ${transaction.txId} => ${boostedTransactionDetails.transactionHash}`)
      if (transaction.attempts > 2) {
        log.debug(`resend ${signer}: Sent tx ${transaction.attempts} times already`)
      }
    }
    return boostedTransactions
  }
